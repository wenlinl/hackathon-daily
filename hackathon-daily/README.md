...
3. 与服务器端历史信息库（`data/archive.json`）查重，重合条目标注"⚠️已收录"，新活动累积入库；
4. 整理成摘要列表（活动名称、主办方、主题、奖金、报名条件、报名/截止/竞赛时间、地点、形式、状态、标签、来源链接）；
5. 写入 Notion：直接在 **2026 数据库（日历视图）** 中创建当天的一条记录
