...
5. 输出大盘环境解读、筛选漏斗、候选榜、候选详解（含板块内领涨个股与自动解读）、
   与昨日对比、待人工逻辑体检，并写入 Notion 当天页面
   （标题 `YYYY-MM-DD 板块扫描报告`，与复盘记录同一日历数据库；同日重跑会整体覆盖）。
...
- 机器分不含"逻辑"维度——催化是否可持续、利好能否落到利润，仍需按报告里的
  "待人工逻辑体检"清单自行判断。
- "与昨日对比"和板块历史缓存都存在 `data/` 目录：本地连续运行直接复用；
  GitHub Actions 通过官方缓存每天恢复/保存，云端同样支持跨日对比与增量更新
  （缓存最长保留 7 天，长期停用后自动回退到全量拉取）。
